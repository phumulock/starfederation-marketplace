...
7. **Casing: the browser lowercases the attribute key, so NEVER write camelCase in it.** HTML attribute *names* are case-insensitive — `data-bind:ackThreshold` reaches Datastar as `data-bind:ackthreshold` before any JS runs. For signal-defining attributes (`data-bind`, `data-signals`, `data-computed`, `data-ref`, `data-indicator`) Datastar reconstructs camelCase *from hyphens* (`data-bind:ack-threshold` → `$ackThreshold`); it cannot recover a hump you typed directly. So `data-bind:ackThreshold` silently binds a *different*, empty signal `$ackthreshold`, while `data-text="$ackThreshold"` — an attribute *value*, which is case-preserved — reads the real one. Symptom: the bound input sits at 0/blank while a sibling label shows the right number (they point at two different signals). **Signal names are camelCase in expressions (`$ackThreshold`) but kebab-case in attribute keys (`data-bind:ack-threshold`, `data-signals:pin-timeout`).** Other-family attributes (`data-class`, `data-attr`, `data-on`, `data-style`) default to kebab instead. Override either default with `__case.camel|kebab|snake|pascal`. Full rules in `references/attributes.md` → Casing.
